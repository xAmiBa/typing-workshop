...
def calculate_total_consumption(readings: list[MeterReading]) -> float:
...
def get_meter_ids(readings: Iterable[MeterReading]) -> list[str]:
    ...
    if readings[0].meter_id.startswith("ELEC"):
        return [r.meter_id for r in readings]
    return []


def reading_generator(meter_id: str, days: int):
    """
...
def process_readings_in_batches(readings: list[MeterReading], batch_size: int):
    """
...
def average_consumption(readings: list[MeterReading]) -> float:
    """
